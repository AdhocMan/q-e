...
!
SUBROUTINE laxlib_compute_distmat_gpu_z( dm, kdim, alpha, v_d, ldv, w_d, ldw, idesc, &
 ...
 status_spla = spla_pzgemm_ssbtr(idesc(LAX_DESC_N), idesc(LAX_DESC_N) - nb1 + 1, kdim, &
          SPLA_OP_CONJ_TRANSPOSE, alpha, c_loc(v_d(1,1)), ldv, c_loc(w_d(1, nb1)), ldw, ONE, &
          c_loc(dm(1,1)), nx, 0, nb1 - 1, SPLA_FILL_MODE_UPPER, mat_dis_spla, ctx_spla)
 IF( status_spla /= SPLA_SUCCESS ) &
   CALL errore( ' laxlib_compute_distmat ',' error when calling SPLA ', ABS(status_spla) )
 !
    ...
    IF( ic+nc-1 >= nb1 ) THEN
       ...
       DO ipr = 1, ipc ! idesc(LAX_DESC_NPR) use symmetry
          ...
          CALL ZGEMM( 'C', 'N', nr, nc, kdim, alpha, v_d(1, ir), &
                      ldv, w_d(1,ii), ldw, ZERO, vtmp_d, nx )
          vtmp = vtmp_d
