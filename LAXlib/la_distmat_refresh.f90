...
SUBROUTINE laxlib_distmat_refresh_z( mdim, ndim, kdim, alpha, v, ldv, dm, idesc, beta,&
 ...
 IF( status_spla /= SPLA_SUCCESS ) &
   CALL errore( ' laxlib_compute_distmat ',' error when calling SPLA ', ABS(status_spla) )
 !
 status_spla = spla_pzgemm_sbs(mdim, ndim, kdim, &
       ...
       DO ipr = 1, idesc(LAX_DESC_NPR)
          ...
          IF( ir <= mdim ) THEN
             ...
             IF( ipr-1 == idesc(LAX_DESC_MYR) .AND. ipc-1 == idesc(LAX_DESC_MYC) .AND. la_proc ) THEN
                ...
                CALL ZGEMM( 'N', 'N', kdim, nc, nr, alpha, &
                         v(1,ir), ldv, dm, nx, beta_loop, w(1,ndim+ic), ldw )
             ELSE
                !
                ...
                CALL ZGEMM( 'N', 'N', kdim, nc, nr, alpha, &
                         v(1,ir), ldv, vtmp, nx, beta_loop, w(1,ndim+ic), ldw )
             END IF
             ! 
             beta_loop = ONE
          END IF
