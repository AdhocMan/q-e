...
program test
  ...
  COMPLEX(DP), pointer :: space_domain(:)
  type(c_ptr) :: space_domain_ptr = c_null_ptr
  !
  !   ........
  !
  ...
  !
  ! Test FFT - First calls may be biased by MPI and FFT initialization
  !
  CALL MPI_BARRIER(MPI_COMM_WORLD, ierr)
  if (use_spfft) then
      spfft_error = spfft_error + spfft_transform_backward(transform, psic, SPFFT_PU_HOST)
      if (spfft_error .ne. SPFFT_SUCCESS) ERROR STOP
      spfft_error = spfft_error + &
        spfft_transform_forward(transform, SPFFT_PU_HOST, psic, SPFFT_NO_SCALING)
      if (spfft_error .ne. SPFFT_SUCCESS) ERROR STOP
  else
    !
